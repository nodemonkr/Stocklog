# StockLog v3.80.1 (2026-08-30)

현재 프로젝트 버전의 단일 기준은 루트 `VERSION` 파일입니다. 웹 메뉴/로그인 화면은 Vite 빌드 시 이 값을 주입받고, FastAPI `/health`, `start-all.sh`, `restart-all.sh`, `check-version.sh`도 같은 값을 사용합니다.

- 버전 확인: `./restart-all.sh --version` 또는 `./check-version.sh`
- 배포 재시작: `./restart-all.sh`
- production 빌드가 성공하면 `frontend/dist/VERSION`이 현재 버전으로 기록됩니다.
- 빌드에 실패했을 때 현재 버전으로 검증되지 않은 오래된 `frontend/dist`를 자동 재사용하지 않습니다.
- v3.80.1 UI 변경: 상승/플러스 빨강, 하락/마이너스 파랑, 스마트 분석 Enter 검색, 수급 핵심 문구 정리, 포트폴리오 밝은 테마 가독성, 계정 테마 선택 제거, 종목상세 감성색 통일.

자세한 변경 내역은 `V3_80_1_CHANGES.md`를 확인하세요.

# Sync Fix 1 — 전체 동기화 99% 정체 보호 (2026-08-30)

전체 동기화의 종목 분류가 2,525/2,525 · 99%에서 장시간 멈추던 OpenDART 무제한 tail retry를 bounded retry + circuit breaker로 변경했습니다. 키움 시세/지표/수급 및 DART 재무에도 공급사 연속 장애 보호를 적용했고, 관리자 polling/heartbeat UI를 정리했습니다. 세부 내용은 `V3_80_0_SYNC_FIX_1.md`를 우선 참고하세요.

# Mobile 3.80.0 native UX/stability baseline (2026-08-30)

모바일 최신 기준은 `mobile/V3_80_0_CHANGES.md`와 `mobile/WEB_PARITY_AUDIT_V3_80_0.md`입니다. 홈 자산 7지표, 스마트 상세 단계적 로딩/Hook crash 수정, 테마 상세 modal, 웹 parity 수급, focus 기반 silent refresh 및 API 자동대조가 포함됩니다.

# StockLog v3.44.0

## 2026-08-28 자동매매 진단 + Gbot 실패학습 메모리 (Mobile 3.79.0)

자동매매의 0건 회차까지 영구 기록하여 `오늘 실행/후보/Gbot 판단/실제 주문/오류/watcher heartbeat/키움·Gbot 성공`을 웹·모바일에서 확인할 수 있습니다.
자동매수 후 손실 사례는 매수 당시 스냅샷과 이후 성과를 분리 저장하고, Gemini 사후감사로 반복 가능한 실패 원인만 `learning_memory`에 축적합니다. 단일 손실은 매수금지 규칙으로 자동 승격하지 않으며, 매수 후 새로 발생한 뉴스/공시는 `post_entry_event`로 구분하여 hindsight bias를 줄입니다.
자동매매 학습 세부구조는 `AUTO_TRADING_DIAGNOSTICS_LEARNING_20260828.md`와 `mobile/V3_79_0_CHANGES.md`를 참고하고, 현재 모바일 전체 기준은 `mobile/V3_80_0_CHANGES.md`를 우선합니다.

## 2026-08-28 Kiwoom 8005 토큰 자동복구 핫픽스

키움이 만료/무효 토큰을 HTTP 401이 아니라 HTTP 200 + `return_code=3` + `8005` 본문으로 반환하는 경우를 처리합니다.
`expires_dt` 기반 선제 갱신, 8005 1회 자동복구/재시도, REST/WebSocket 토큰 공유를 적용했습니다.
세부 내용은 `KIWOOM_TOKEN_8005_FIX_20260828.md`를 우선 참고하세요.


> v3.44.0: 스마트 종목 분석 화면을 2개 추천 유형으로 단순화하고, 시장지표를 축소했으며 추천 후보를 최대 20개로 제한했습니다. `V3_41_CHANGES.md`를 확인하세요.


기존 StockLog의 스마트 분석 API 흐름을 유지하면서 아래 기능을 한 번에 포함한 전체 교체용 버전입니다.

## 포함 기능

- 로그인 / 회원가입 / JWT 인증
- 최초 관리자: `admin / admin`
- 사용자별 키움 REST API 모의투자 설정
- 키움 모의투자 연결 테스트 / 계좌 조회
- 모의투자 매수 / 매도 주문
- 보유자산 / 보유종목 / 수익률 조회
- 관리자 종목 유니버스 동기화
- `/api/smart/value` 호환 API
- 스마트 분석 카테고리 분류
- 종목 상세 종합차트
- 최신 뉴스 + 긍정/중립/부정 간이 감성분석
- 분기 재무정보
- 종합 투자 점수 / 추천·관망·비추천 및 이유
- 동기화 중복 실행 방지 / 쿨다운
- Naver News API 선택 연동
- OpenDART 선택 연동
- Kiwoom REST API 모의투자 선택 연동
- 실데이터 우선 원칙: Kiwoom/OpenDART/실제 수집 데이터를 사용하며 임의 데모 시장데이터를 생성하지 않음
...
처음 실행은 frontend의 npm 패키지 설치 때문에 시간이 조금 걸릴 수 있습니다.
...
기존 StockLog v3.39.0 데이터베이스를 유지한 상태로 업그레이드할 수 있습니다. 새 외부 API 설정/사용량 테이블은 시작 시 자동 생성됩니다. 운영 반영 전에는 기존 StockLog 폴더와 MySQL DB를 백업하세요.

기존 프로젝트에서 유지해야 할 핵심 호환 API:

```text
GET /api/smart/value
```

자세한 적용은 `APPLY_GUIDE.md`를 확인하세요.
...
- 거래량 차트
- KOSPI 비교선
- 마우스 hover 시 상승일 빨간 툴팁 / 하락일 파란 툴팁
- 시가/고가/저가/종가/거래량/이동평균/KOSPI 상세값 표시
- 분기별 재무의 전분기 대비 증감률 및 ↑/↓ 시각화
- 사업성과 / 재무 / 밸류 / 시장·뉴스 / 향후 방향성으로 심층 추천 근거 분리
- 뉴스 클릭 시 새 탭 원문 열기
...
- 사용자는 App Key / Secret Key만 입력
- 저장 시 키움 모의투자 토큰 발급
- ka00001 계좌번호 자동 조회
- 조회된 계좌번호 사용자 DB 자동 저장
- 계좌번호 수동 입력 UI 제거
- 기존 DB에서 계좌번호가 비어 있어도 최초 잔고조회 시 자동 보정
- 국내주식 계좌평가잔고 TR을 kt00017로 수정
- 키움 client/token을 프로세스 내에서 재사용하여 불필요한 토큰 발급 감소


## v3.5 키움 모의계좌 직접 동기화

StockLog 자체 가상 장부를 사용하지 않습니다.

- 계좌번호: 키움 `ka00001`
- 주문: 키움 `kt10000 / kt10001`
- 잔고/예수금/체결: mock 계정에서 실제로 성공하는 계좌 TR을 자동 탐색
- 미지원 TR (`RC9000`)은 자동 제외
- 성공한 TR과 실패한 TR을 모의투자 화면의 `동기화 진단`에서 표시
- 마지막 성공 계좌정보는 DB 캐시로 저장
- 영웅문/다른 클라이언트에서 같은 모의계좌로 거래해도 다음 강제 동기화 때 키움 원장 기준으로 반영 가능
...
계정마다/모의환경 정책에 따라 지원 여부가 다를 수 있기 때문에
고정 TR 하나가 아니라 실제 성공 응답을 기준으로 자동 선택합니다.

### 중요한 점

이 버전은 키움 mock 서버가 반환하는 데이터를 원본으로 사용합니다.
따라서 계좌 TR 중 적어도 하나가 해당 모의계정에서 지원되어야
총 자산/보유종목 등을 표시할 수 있습니다.

모든 후보가 RC9000이면 StockLog가 임의로 가상잔고를 만들지 않고,
동기화 진단에 그대로 표시합니다.


## v3.6 모의투자 호출 제한 수정

키움 모의투자는 동일 TR에 대해 국내주식 기준 1초당 1회 제한이 추가로 적용됩니다.

v3.5에서는 하나의 TR을 여러 Body로 약 0.28초 간격으로 시험해
`HTTP 429`를 유발할 수 있었습니다.

v3.6에서는:

- 동일 TR 최소 1.10초 간격
- 전체 REST 요청도 최소 간격 적용
- 429 발생 시 1.25초 후 동일 요청 1회만 재시도
- 재시도도 429면 해당 TR의 다음 Body 시도를 중단
- 다른 TR은 계속 진단
- 429를 `미지원`과 별도로 표시
...
## v3.29 — 실시간 모의투자 포트폴리오 / 외부 접속

### 실시간 가격과 수익
- 보유수량·평균단가·예수금·주문/체결은 키움 모의계좌 원장을 사용합니다.
- 보유종목의 현재가는 키움 국내주식 실시간 `주식체결(0B)` WebSocket을 서버에서 구독합니다.
- 브라우저에는 StockLog WebSocket `/ws/kiwoom/portfolio-live`를 통해 전달합니다.
- 실제 체결 tick이 들어올 때마다 현재가, 평가금액, 평가손익, 수익률, 당일 가격변동 효과를 즉시 다시 계산합니다.
- synthetic/random 가격은 사용하지 않습니다.
- 실시간 tick으로 최근 흐름 mini sparkline을 표시합니다.
- 예수금/보유수량/주문내역 같은 계좌 원장은 30초마다 background sync하며 화면을 막지 않습니다.

### 화면
- 총 자산 / 평가손익 / 수익률 / 오늘 보유종목 변동
- 예수금 / 매입금액 / 실시간 평가액 / 보유종목 수
- 보유종목별 카드: 현재가, 수량, 평균단가, 평가액, 손익, 수익률, 당일 변동, 포트폴리오 비중
- 실시간 연결 상태와 최근 수신 시간
- 주문, 실제 일봉 차트, 주문/체결 내역, TR 진단

### 외부 네트워크
기본 bind는 `0.0.0.0` 입니다.

```bash
cp stocklog.env.example stocklog.env
vi stocklog.env
./restart.sh
```
...
```bash
STOCKLOG_BIND_HOST=0.0.0.0
STOCKLOG_FRONTEND_PORT=5174
STOCKLOG_BACKEND_PORT=8100
STOCKLOG_PUBLIC_HOST=203.0.113.10
```
...
공유기/NAT 환경에서는 외부에서 직접 접속하려면 TCP 5174와 8100을 StockLog 서버로 포트 포워딩해야 합니다. 공인 IP가 없거나 CGNAT 환경이면 일반 포트 포워딩만으로는 직접 접속할 수 없으므로 별도 VPN/터널/리버스프록시 구성이 필요할 수 있습니다.

HTTPS reverse proxy를 사용하는 경우 `frontend/.env.local`에 다음처럼 설정할 수 있습니다.
